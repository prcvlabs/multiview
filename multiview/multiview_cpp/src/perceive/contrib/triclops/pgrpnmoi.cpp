...
//
// getToken() -  this function gets a whitespace delineated描绘 token out
...
static bool getInt(FILE* stream, int* val)
{
   ...
   if(!getToken(stream, buf, 512)) return false;
...
static bool getComment(FILE* stream, char* szComment)
{
   ...
   if(szComment == NULL) { return true; }

   // get all of the white space characters up to the '#'
   ch = fgetc(stream);

   while(isspace(ch) && ch != EOF) { ch = fgetc(stream); }
...
static bool getCommentLine(char** stream, char* szCommentLine)
{
   ...
   // make sure we have a valid pointer for a string to be stored.
   if(szCommentLine == NULL) { return true; }

   // get all of the white space characters up to the '#'
   int indx = 0;
   ch       = (int) (*stream)[indx];

   while(isspace(ch) && ch != EOF) { ch = (int) (*stream)[++indx]; }
...
//
// ppmSidebySideto32bitPacked this function is called to convert a side by side
// ppm image to normal 32 bit triclops input ready image
//
bool ppmSidebySideto32bitPacked(int nrows,
                                int ncols,
                                unsigned char* data,
...
bool ppmIsSideBySide(const char* filename)
{
   ...
   if(ncols > (nrows * 2)) // if this is a side by side image ,then num of cols
                           // should be greater than 2* num of rows.
   {
      free(data);
      return true;
   } else {
...
//
bool readPGM8BinaryData(FILE* stream, int nrows, int ncols, unsigned char* data)
{
   size_t len = fread(data, 1, nrows * ncols, stream);
   if((size_t) nrows * ncols != len) return false;
...
// 8-bit-pixel pgm file into the provided buffer
//
bool readPGM8AsciiData(FILE* stream, int nrows, int ncols, unsigned char* data)
{
   for(int row = 0; row < nrows; row++) {
      for(int col = 0; col < ncols; col++) {
         if(!getByte(stream, data++)) { return false; }
      }
   }
   return true;
}
...
//
bool readPPM8BinaryPackedRGB(FILE* stream,
                             int nrows,
                             int ncols,
                             unsigned char* data)
{
   for(int row = 0; row < nrows; row++) {
      for(int col = 0; col < ncols; col++) {
         // get red byte
         if(!fread(data + 0, 1, 1, stream)) { return false; }

         // get green byte
         if(!fread(data + 1, 1, 1, stream)) { return false; }

         // get blue byte
         if(!fread(data + 2, 1, 1, stream)) { return false; }
...
// Consequently there are 32 bits per pixel.
//
bool readPPM8BinaryPacked(FILE* stream,
                          int nrows,
                          int ncols,
                          unsigned char* data)
{
...
bool readPPM24BinaryPackedBGR(FILE* stream,
                              int nrows,
                              int ncols,
                              unsigned char* data)
{
...
// Consequently there are 32 bits per pixel.
//
bool readPPM8AsciiPacked(FILE* stream,
                         int nrows,
                         int ncols,
                         unsigned char* data)
{
   for(int row = 0; row < nrows; row++) {
      for(int col = 0; col < ncols; col++) {
         // get red byte
         if(!getByte(stream, data + 2)) { return false; }

         // get green byte
         if(!getByte(stream, data + 1)) { return false; }

         // get blue byte
         if(!getByte(stream, data + 0)) { return false; }
...
// Consequently there are 32 bits per pixel.
//
bool readPPM8AsciiPackedRGB(FILE* stream,
                            int nrows,
                            int ncols,
                            unsigned char* data)
{
...
//
bool readPPM8BinaryRGB(FILE* stream,
                       int nrows,
                       int ncols,
                       unsigned char* red,
                       unsigned char* green,
                       unsigned char* blue)
{
   for(int row = 0; row < nrows; row++) {
      for(int col = 0; col < ncols; col++) {
         // get red byte
         if(!fread(red, 1, 1, stream)) { return false; }

         // get green byte
         if(!fread(green, 1, 1, stream)) { return false; }

         // get blue byte
         if(!fread(blue, 1, 1, stream)) { return false; }
...
// Consequently there are 32 bits per pixel.
//
bool readPPM8AsciiRGB(FILE* stream,
                      int nrows,
                      int ncols,
                      unsigned char* red,
                      unsigned char* green,
                      unsigned char* blue)
{
   for(int row = 0; row < nrows; row++) {
      for(int col = 0; col < ncols; col++) {
         // get red byte
         if(!getByte(stream, red)) { return false; }

         // get green byte
         if(!getByte(stream, green)) { return false; }

         // get blue byte
         if(!getByte(stream, blue)) { return false; }
...
MachineEndianness pgrGetEndianness(void)
{
...
//
static bool
readPGM16BinaryData(FILE* stream, int nrows, int ncols, unsigned short* data)
{
   size_t len_read;
   ...
   if(len_expected != len_read) { return false; }

   /* see if we need to swap data */
   endianness = pgrGetEndianness();
   if(PGR_ENDIANNESS_BIG == endianness) { return true; }

   /* we are running on little endian arch
      so byte swap is necessary */
   for(i = 0; i < int64_t(len_expected); ++i, ++data) {
      val   = *data;
...
//
static bool
readPGM16AsciiData(FILE* stream, int nrows, int ncols, unsigned short* data)
{
   uint32_t len_expected;
   MachineEndianness endianness;
   uint16_t val;
...
//
bool pgm8Read(const char* filename,
              char* szComment,
              int* nrows,
              int* ncols,
              unsigned char** data)
...
//
bool pgm16Read(const char* filename,
               char* szComment,
               int* nrows,
               int* ncols,
               unsigned short** data)
...
//  ie: [R G B U][R G B U]...
//
bool ppm8ReadPacked(const char* filename,
                    char* szComment,
                    int* nrows,
                    int* ncols,
                    unsigned char** data)
{
   ...
   *data = (unsigned char*) malloc(4 * (*ncols) * (*nrows)
...
//
bool ppm8ReadRGB(const char* filename,
                 char* szComment,
                 int* nrows,
                 int* ncols,
                 unsigned char** red,
...
//
//
bool parsePGMHeader(FILE* stream,
                    char* szComment,
                    int* nrows,
                    int* ncols,
                    int* maxval,
                    bool* ascii)
{
   ...
   if(fgetc(stream) != 'P') { return false; }
...
//  ascii (indicates the format of the input)
//
bool parsePPMHeader(FILE* stream,
                    char* szComment,
                    int* nrows,
                    int* ncols,
                    int* maxval,
