...
//===========================================================================
void SLIC::DoRGBtoLABConversion(const uint32_t*& ubuff,
                                double*& lvec,
                                double*& avec,
                                double*& bvec)
{
...
/// For whole volume
//===========================================================================
void SLIC::DoRGBtoLABConversion(uint32_t**& ubuff,
                                double**& lvec,
                                double**& avec,
                                double**& bvec)
{
...
//=================================================================================
void SLIC::DrawContoursAroundSegments(uint32_t*& ubuff,
                                      int*& labels,
                                      const int& width,
                                      const int& height,
   ...
   for(int n = 0; n < numseeds; n++) {
      ...
      for(int i = 0; i < 8; i++) {
         ...
         if(nx >= 0 && nx < m_width && ny >= 0 && ny < m_height) {
            ...
            if(edges[nind] < edges[storeind]) { storeind = nind; }
         }
      }
      if(storeind != oind) {
                                ...
                                const string& path)
{
   ...
   string newname = nameandextension.replace(
       nameandextension.rfind(".") + 1,
       3,
       "dat"); // find the position of the dot and
               // replace the 3 characters following it.
    ...
    const int& STEP)
{
         ...
         for(int h = 0; h < height; h++) {
            for(int w = 0; w < width; w++) {
               if(nlabels[d][i] < 0) {
                  ...
                  // adjacent label found before, and decrement label count.
                  //-------------------------------------------------------
                  if(count <= (SUPSZ >> 2)) // this threshold can be changed
                                            // according to needs
                  {
                     for(int c = 0; c < count; c++) {
                        int ind               = yvec[c] * width + xvec[c];
   ...
   {
      for(int d = 0; d < depth; d++) { delete[] nlabels[d]; }
   }
   delete[] nlabels;
   //------------------
   if(xvec) delete[] xvec;
    ...
    const double& compactness)
{
   ...
   // klabels.resize( sz, -1 );
   //--------------------------------------------------
   if(klabels == nullptr) // Added AJM: we don't allocate memory
      klabels = new int[sz];
   ...
   //--------------------------------------------------
   bool perturbseeds(false); // perturb seeds is not absolutely necessary, one
...
//===========================================================================
///	DoSuperpixelSegmentation_ForGivenSuperpixelSize_LAB
///
/// Takes a pointer to array of Vector3fs containing the L*A*B* values of
/// the pixels
///
/// compactness value depends on the input pixels values. For instance, if
/// the input is greyscale with values ranging from 0-100, then a compactness
...
/// The labels can be saved if needed using SaveSuperpixelLabels()
//===========================================================================
void SLIC::DoSuperpixelSegmentation_ForGivenSuperpixelSize_LAB(
    const perceive::Vector3f* pixels,
    const int width,
    const int height,
    int*& klabels,
    int& numlabels,
    ...
    const double& compactness)
{
   ...
   m_avec = new double[sz];
   m_bvec = new double[sz];
   const perceive::Vector3f* in_pixel{pixels};
   int out_idx{0};
   for(int row{0}; row < height; ++row) {
      for(int col{0}; col < width; ++col) {
         m_lvec[out_idx] = in_pixel->x;
         m_avec[out_idx] = in_pixel->y;
         m_bvec[out_idx] = in_pixel->z;
         ++out_idx;
         ++in_pixel;
      }
   }
    ...
    const double& compactness) // weight given to spatial distance
{
   ...
   DoSuperpixelSegmentation_ForGivenSuperpixelSize(
       ubuff, width, height, klabels, numlabels, superpixelsize, compactness);
}
...
//===========================================================================
void SLIC::DoSupervoxelSegmentation(uint32_t**& ubuffvec,
                                    const int& width,
                                    const int& height,
                                    const int& depth,
                                    int**& klabels,
                                    ...
                                    const double& compactness)
{
   ...
   for(int d = 0; d < depth; d++) {
      ...
      for(int s = 0; s < sz; s++) { klabels[d][s] = -1; }
   }
   ...
   EnforceSupervoxelLabelConnectivity(
       klabels, width, height, depth, numlabels, STEP);
}
