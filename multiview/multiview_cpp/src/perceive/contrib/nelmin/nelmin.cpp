...
namespace nelmin
{
void set_nelder_mead_feedback(bool value) { feedback = true; }
...
*/
{
   ...
   double ystar;
   double z;
   double last_ynewlo = std::numeric_limits<double>::max(); // AJM
   ...
   */
   for(;;) {
      ...
      for(;;) {
         ...
         */
         if(*icount <= kcount) {
            ...
            if(feedback)
               printf("\x1b[43mynewlo = %f, reqmin=%f, dn=%f, rq=%f, "
                      "z=%f, restarts=%d, icount=%d\x1b[0m\n",
                      *ynewlo,
                      reqmin,
                      dn,
                      rq,
      ...
      *ynewlo = y[ilo];
      ...
      if(kcount < *icount) {
         *ifault = 2;
         if(feedback) printf("\x1b[43mKCOUNT exceeded\x1b[0m\n");
         break;
      }
      ...
      *ifault = 0;
      ...
      if(*ifault == 0) {
         if(feedback) printf("\x1b[43mMINIMUM found\x1b[0m\n");
         break;
      }

      if(fabs(last_ynewlo - *ynewlo) < reqmin) {
         *ifault = 3;
         if(feedback)
            printf("\x1b[43mREQMIN hit (|%f - %f| = %f, %f\x1b[0m\n",
                   last_ynewlo,
                   *ynewlo,
                   fabs(last_ynewlo - *ynewlo),
                   *ynewlo);
         break;
      }

      if(max_restarts >= 0 && *numres >= max_restarts) {
         *ifault = 4;
         if(feedback) printf("\x1b[43mMAX-RESTARTS hit\x1b[0m\n");
         break;
      }
      ...
      */
      ...
      del     = eps;
      *numres = *numres + 1;
      if(feedback) printf("\x1b[43mRESTART\x1b[0m\n");

      last_ynewlo = *ynewlo; // AJM
   }
